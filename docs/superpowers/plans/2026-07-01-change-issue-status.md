# change_issue_status Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a `change_issue_status` composite MCP tool that resolves an issue key and status code, then PATCHes the issue.

**Architecture:** New `_resolve_status_code` helper queries `DirectoryEntries` for matching status code (case-insensitive). `_change_issue_status` reuses existing `_get_issue_by_key` for issue resolution, calls `client.update`, returns compact JSON. Follows the same pattern as `create_issue` / `link_issues` / `attach_file`.

**Tech Stack:** Python 3.10+, httpx, FastMCP, pytest, respx

## Global Constraints

- Single pydoc line for every `@mcp.tool()` (existing convention)
- Status resolution from `DirectoryEntries` — same catalog entity as `_resolve_priority`
- Issue resolution via `_get_issue_by_key` (existing function)
- Response format: `{"changed": true, "id": "...", "key": "...", "statusCode": "..."}`
- Error prefix: `"Error: ..."` on failure
- Test pattern: `CompositeFakeClient` with `monkeypatch` (existing convention)
...
async def test_change_issue_status_unknown_key_returns_error(monkeypatch):
    fake = CompositeFakeClient()
    # Override query to return empty for unknown key
    orig = fake.query
...
async def test_change_issue_status_numeric_id_fallback(monkeypatch):
    ...
    # First query returns nothing (not found by code), get_by_id should be called
